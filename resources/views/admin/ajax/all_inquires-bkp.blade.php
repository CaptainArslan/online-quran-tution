    ...
    <table class="table   table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td> <a href="{{--@if($student->status!=='pending') {{route('admin.student.edit.schedule',$student->id)}} @else # @endif --}}#" title="Edit Schedule"> {{$student->user->name ?? 'N / A'}}</a></td>
                <td> {{$student->child->name ?? 'N / A'}}</td>
                <td> {{$student->user->phone ?? 'N / A'}} </td>
                <td> {{$student->user->email ?? 'N / A'}} </td>
                <td> {{$student->child->skype_id ?? 'N / A'}} </td>
                <td>
                    @if ($student->child)
                    {{ \Carbon\Carbon::parse($student->child->skype_assigned_at)->format('Y-m-d') ?? 'N / A' }}
                    @else
                    N / A
                    @endif
                </td>
                ...
                --}}
                <td>
                    @if(is_null($student->user->email_verified_at))

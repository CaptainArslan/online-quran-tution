        ...
        <div class="col-sm-8 m-auto">
            <form method="GET" action="{{ route('admin.expense.list') }}" class="filter-form">
                <div class="input-group">
                    <input type="text" name="from" class="form-control datepicker filter"
                        value="{{ $req->from ?? '' }}" placeholder="From">
                    <input type="text" name="to" class="form-control datepicker  filter"
                        value="{{ $req->to ?? '' }}" placeholder="To">
                    <select class="form-control filter" name="filter_type">
                        <option default selected>Select Filter</option>
                        <option @if ($req->filter_type) {{ $req->filter_type == 'daily' ? 'selected' : '' }} @endif
                            value="daily">Daily</option>
                        <option @if ($req->filter_type) {{ $req->filter_type == 'weekly' ? 'selected' : '' }} @endif
                            value="weekly">Weekly</option>
                        <option @if ($req->filter_type) {{ $req->filter_type == 'monthly' ? 'selected' : '' }} @endif
                    ...
                    <div class="input-group-append">
                        ...
                        <button type="submit" name="export" value="export"
                            class="btn btn-info reset pl-1 pr-1">Export</button>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <tbody>
                                @foreach ($expense as $ex)
                                    <tr>
                                        ...
                                        <td>
                                            @if (isset($ex->receipt))
                                                <a href="{{ asset($ex->receipt) ?? '' }}" target="_blank"><img
                                                        src="{{ asset($ex->receipt) ?? '' }}"
                                        ...
                                        <td>
                                            <div class="btn-group pull-right">
                                                ...
                                                <button type="button"
                                                    onclick="deleteAlert('{{ route('admin.expense.delete', $ex->id) }}')"

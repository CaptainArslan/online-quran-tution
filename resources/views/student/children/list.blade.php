                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-primary">
                                            @php
                                                if(isset($child->inquiry->is_paid)){
                                    ...
                                    <td>
                                        @if(isset($child->inquiry->status))
                                            @if (isset($child->inquiry->status) && $child->inquiry->status == 'pending')
                                    ...
                                    <td>
                                        @if(count($child->subscriptions) > 0)
                                            @forelse ($child->subscriptions as $subscription)
                                    ...
                                    <td>
                                        <div class="buttonDiv">
                                            <a class="btn btn-success"
                                                href="{{ route('student.child.schedule', ['id' => base64_encode($child->id)]) }}">View
                                                Schedule</a>
                                        </div>
                                    </td>
                                    <td>
                                        <a class="btn btn-warning"
                                            href="{{ route('student.multiple.subscription', ['id' => base64_encode($child->id)]) }}">Add Plan</a>
                                    </td>
                                    <td>
                                        <div class="buttonDiv d-flex justify-content-around align-items-center">
                                            <a
                                                href="{{ route('student.update.children', ['id' => base64_encode($child->id)]) }}"><i
                                                    class="feather icon-edit"
                                                    style="font-size: 24px; color: royalblue"></i></a>
                                            <a
                                                onclick="deleteAlert('{{ route('student.delete.children', $child->id) }}')"><i
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form name="contactForm" method="post"
                                        action="{{ route('student.child.login') }}">
                                        @csrf

          ...
          <table class="table table-striped table-bordered table-striped datatable">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <div class="btn-group">
                    <a href="{{url('admin/tutor/'.$st->id ?? '')}}" class="btn btn-primary">View/Edit</a>
                    <button type="button" onclick="deleteAlert('{{ route('admin.remove_tutor', $st->id) }}')" class="btn btn-danger">Delete</button>
                  </div>
                </td>
                <td>
                  <a href="{{route('admin.tutor.payout',$st->id)}}" class="btn btn-info">Payout</a>
                </td>
                <td width="15%">
                  <a href="{{ route('admin.tutor.inquiries', $st->id) }}" class="btn btn-success">Inquiries</a>
                </td>
                <td class="text-center">
                  @if(isset($st->tutor->document))
                  <a href="{{ asset($st->tutor->document) }}" target="_blank" title="Download Documents"><i class="fas fa-file fa-2x text-info"></i></a>
                  @else

                ...
                <div class="col-md-6 col-sm-6">
                    <div class="card plan-card" data-plan="{{ $pl->id }}">
                        ...
                        <div class="text-center bg-success text-white p-1">
                            @if ($pl->discount == 0)
                                <h3 class="text-white font-weight-bold pb-0 mb-0">
                                    {{ $pl->country->currency }}{{ $pl->price_per_month ?? '' }} Price Per Month </h3>
                            @else
                                <h4 class="text-white font-weight-bold"><strike>Start from {{ $pl->country->currency }}
                                        {{ $pl->price }}</strike> </h4>
                                <h3 class="text-white font-weight-bold">Discount Price</h3>
                                <h1 class="text-white font-weight-bold">{{ $pl->country->currency }} @php echo $discounted_price = $pl->price - $pl->discount;  @endphp
                                </h1>
                            @endif

                        </div>
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item"><i class="fa fa-check-square"></i>{{ $pl->days_in_week ?? '' }}
                                Days in Week</li>
                            <li class="list-group-item"><i
                                    class="fa fa-check-square"></i>{{ $pl->classes_in_month ?? '' }} Classes in Month
                            </li>
                            <li class="list-group-item"><i class="fa fa-check-square"></i>{{ $pl->duration ?? '' }}
                                Duration</li>
                        </ul>
                ...
                <form action="{{ route('student.payments.create') }}" method="POST">
                    ...
                    <div class="container p-0" id="gocardless-description">
                        ...
                        <div class="card-body pt-0">
                            <p><strong>Go Cardless Payments protected under the Direct Debit Guarantee</strong>The
                                Direct Debit Guarantee protects your customers against payments made in error or
                                fraudulently,
                                making Direct Debit the UKs safest payment method.</p>
                            <div>
                                <img src="{{ asset('images/gocardless.jpg') }}" class="img-fluid" alt="gocardless">
                            </div>
                            <a href="https://gocardless.com/direct-debit/guarantee/" target="_blank"
                                class="btn pl-0 pt-2 text-info">https://gocardless.com/direct-debit/guarantee/</a>
                            <h4 class="bg-info text-white p-1">From individuals to multi-national corporations,
                                ...
                                <a href="https://gocardless.com/stories/" class="text-dark"
                                    target="_blank">https://gocardless.com/stories</a>
                            </h4>

                            <div class="mt-2">
                                <h3>Fully safe and secure</h3>
                                <p>GoCardless is authorised by the Financial Conduct Authority, and ISO27001 certified
                                    for security standards.</p>

                            </div>
                        </div>
                        <div class="card-footer">
                            <small>GoCardless Ltd., Sutton Yard, 65 Goswell Road, London, EC1V 7EN, United Kingdom
                                GoCardless (company registration number 07495895) is authorised by the Financial Conduct
                                Authority under the Payment Services Regulations 2017, registration number 597190, for
                                the provision of payment services.<br />If you have any issue in paymetns with
                                GoCardLess? <a href="{{ route('index') }}" class="font-weight-bold">click here</a> to
                                contact.
                        </div>
                        </small>
                    ...
                    <div class="row mb-2 pt-2">
                        <div class="col-sm-12 mb-2">
                            <a class="font-weight-bold text-dark promo-code-section" data-toggle="collapse"
                                href="#collapseExample" role="button" aria-expanded="false"
                                aria-controls="collapseExample"><u>Have a promo code?</u></a>
                            <div class="collapse" id="collapseExample">
                                <label class="text-danger" id="response-msg"></label>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="promo" name="promo"
                                        placeholder="promo code e.g. MBL350"

    ...
    public function tutor($id = null)
{
    ...
    if ($id != null) {
        ...
        // Calculate paid students in a month and total trials in the month
        $currentMonth = date('m');
        $trial_month_query = Inquiry::where('tutor_id', $id)
            ->where('is_paid', false)
            ->whereMonth('created_at', $currentMonth);
        
        $trial_month = $trial_month_query->get();
        $total_trials_in_month = $trial_month_query->count();

        foreach ($trial_month as $trial) {
            if (Subscription::where('inquiry_id', $trial->id)->where('status', 'active')->count() > 0) {
                $paid_students++;
            }
        }

        // Calculate conversion rate
        $conversion_rate = ($total_trials_in_month > 0) ? ($paid_students / $total_trials_in_month) : 0;
    }
    ...
    $data = [
        ...
        'total_trials_in_month' => $total_trials_in_month,
        'conversion_rate' => $conversion_rate,
    ];
    // dd($total_trials_in_month);
    ...
    public function tutor_list(Request $request)
    {
        ...
        foreach ($data as $tutor) {
            ...
            $trial_month_query = Inquiry::where('tutor_id', $tutor_id)
                ->where('is_paid', false)

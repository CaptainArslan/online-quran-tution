...
class InquiryController extends Controller
{
    ...
    //             ob_end_clean();
    //             $response = Excel::download(new AllInquiriesExport($searchResults), 'all_Inquiries.xlsx' , \Maatwebsite\Excel\Excel::XLS);
    //             return $response;
    //         }
    ...
    {
        if ($req->action == 'export') {
            return $this->exportInquiries($req);
        }

        if ($req->ajax()) {
            $searchResults = $this->getFilteredSearchResults($req)
            ->paginate($req->table_length_limit);
            return view('admin.ajax.all_inquiries', compact('searchResults'));
        }

        return view('admin.inqueries.all_inquiries', get_defined_vars());
    }

    private function getFilteredSearchResults($req)
    {
        $searchResults = $this->SearchModel('Inquiry', 'user', ['inquiry', 'tutor_id'], ['name', 'email', 'phone'], $req->table_filter_search ?? '')
            ->where('status', '!=', 'cancelled')
            ->when(auth()->user()->role == 'manager', function ($query) {
                return $query->where('is_interested', 1)->orderBy('id', 'DESC');
            }, function ($query) {
                return $query->orderBy('id', 'DESC');
            })
            ->when($req->filter_type, function ($query) use ($req) {
                return $this->applyDateFilter($query, $req->filter_type);
            })
            ->when($req->status && $req->status !== 'All Status', function ($query) use ($req) {
                return $query->where('status', $req->status);
            })
            ->when(isset($req->from) && isset($req->to), function ($query) use ($req) {
                return $query->whereBetween('created_at', [$req->from, $req->to]);
            }, function ($query) use ($req) {
                return $query->when(isset($req->from) && !isset($req->to), function ($query) use ($req) {
                    return $query->where('created_at', '>', $req->from);
                }, function ($query) use ($req) {
                    return $query->when(isset($req->to), function ($query) use ($req) {
                        return $query->where('created_at', '<', $req->to);
                    });
                });
            })
            ->when(isset($req->duration), function ($query) use ($req) {
                return $query->where('duration', $req->duration);
            });
    ...
    private function exportInquiries($req)
    {
        $searchResults = $this->getFilteredSearchResults($req)->get();
        ob_end_clean();
